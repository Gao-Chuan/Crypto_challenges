...
// 没写socket IO 的部分.涉及到socket 输入输出的地方直接赋值了. 这个wp是直接对题目原文修改得来的, 所有的修改部分都用 // WP EDIT 与 // WP EDIT END 夹住了. 所有的赋值不涉及选手未知的内容. bg 和 bh 在题目最开始的输出中给出了.
import (
	"bytes"
	"crypto/elliptic"
	"crypto/sha256"
...
func handleClient(conn net.Conn, bg, bh, key []byte) {
	...
	tmp_signedFromChallenger := make([]byte, 1024)
	conn.Write([]byte("\nShow me your original signature:>>"))
	readLen, err := conn.Read(tmp_signedFromChallenger)
	tmp_signedFromChallenger, err = hex.DecodeString(string(tmp_signedFromChallenger[:readLen-1]))
	checkError(err)
	signedFromChallenger = []byte(hex.EncodeToString(Ecmac([]byte("admin"), tmp_signedFromChallenger, bg, bh)) + "\n")
	//  WP EDIT END```
	...
	nmreadLen := len(append(_padding([]byte("a\n")), []byte("admin")...))
	challengerFakeName = append(_padding([]byte("a\n")), []byte("admin")...)
	// WP EDIT END
	if err != nil {
		return
	}
